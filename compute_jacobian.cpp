    ...
    protected:
	  ...
	  public:
	   Controller(const rclcpp::NodeOptions& opt = rclcpp::NodeOptions()) : Node("Controller", opt)
    	{
    		Eigen::Vector3d reference_point_position_ = Eigen::Vector3d::Zero(); //servirà per il calcolo dello jacobiano
	        ...
	        velocity_cmd_pub_= this->create_publisher<sensor_msgs::msg::JointState>("/joint_group_velocity__controller/command", 1);
   ...
   void timer_cb()
    {
        ...
        pubblica_msg.velocity.resize(joint_names_.size());
