...
#include "pkg_traccia_temperatura/quintic.hpp"

class JointTrajActionServer : public rclcpp::Node
{
  using JointTraj = temperatura_interfaces::action::Traj;
  using GoalHandleJointTraj = rclcpp_action::ServerGoalHandle<JointTraj>;
  
  protected:
    rclcpp_action::Server<JointTraj>::SharedPtr traj_action_server_;  //action server per la action joint_traj
  ...
  public:
  JointTrajActionServer(const rclcpp::NodeOptions opt = rclcpp::NodeOptions()) : Node("joint_traj", opt)
  {
 ...
 rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID& uuid,
  ...
  void execute(const std::shared_ptr<GoalHandleJointTraj> goal_handle)
  {
    ...
    // La traiettoria sarà pubblicata con una certa frequenza (100 Hz in questo caso)
